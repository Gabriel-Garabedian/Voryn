...
const ACHIEVEMENTS = [
  ...
  { id:'week1',         icon:'04', title:'Uma Semana',        desc:'Treine 7 dias seguidos',                     check: (m) => m.bestStreak >= 7 },
  { id:'streak14',      icon:'05', title:'Duas Semanas',      desc:'Treine 14 dias seguidos',                    check: (m) => m.bestStreak >= 14 },
  { id:'workouts50',    icon:'06', title:'50 Treinos',         desc:'Complete 50 treinos no total',               check: (m) => m.total >= 50 },
  { id:'vol1k',         icon:'07', title:'1 Tonelada',        desc:'Levante 1.000 kg em volume total',            check: (m) => m.totalVolume >= 1000 },
  { id:'week4',         icon:'08', title:'Mês Consistente',   desc:'Treine 3×/semana por 4 semanas',             check: (m) => m.monthlyCount >= 12 },
  { id:'month1',        icon:'09', title:'Um Mês Seguido',    desc:'Treine 30 dias seguidos',                    check: (m) => m.bestStreak >= 30 },
  { id:'workouts100',   icon:'10', title:'100 Treinos',        desc:'Complete 100 treinos no total',              check: (m) => m.total >= 100 },
  { id:'vol10k',        icon:'11', title:'10 Toneladas',      desc:'Levante 10.000 kg em volume total',           check: (m) => m.totalVolume >= 10000 },
  { id:'vol100k',       icon:'12', title:'100 Toneladas',     desc:'Levante 100.000 kg em volume total',          check: (m) => m.totalVolume >= 100000 },
]
...
function AchievementCard({ achievement, unlocked, isNew }) {
  ...
  return (
    <div ref={ref} className="f-card p-4 flex items-center gap-4 transition-all"
      ...
      <div className="achievement-mark w-14 h-14 flex items-center justify-center flex-shrink-0 text-xl relative"
        style={{
...
export default function AchievementsView() {
  ...
  if (loading) return (
    <div className="app-view view-achievements px-4 pt-6 pb-8">
      <p className="view-kicker">Milestones / 07</p>
  ...
  return (
    <div className="px-4 pt-6 pb-8">
      ...
      {unlocked.length > 0 && (
        <div className="mb-5">
          <p className="f-label mb-3">01 · Desbloqueadas ({unlocked.length})</p>
      ...
      {locked.length > 0 && (
        <div>
          <p className="f-label mb-3">02 · Bloqueadas ({locked.length})</p>
          <div className="space-y-2">
